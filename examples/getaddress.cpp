/*
 * getaddress: Get address from wallet test program
...
static int getAddresses(wallet::walletDev *wallet, const std::string &coin, const std::string &derivationPath,
                        wallet::inputScriptType scriptType, int count, bool display, bool usePassphrase) {
    std::list<std::string> addresses;

    printf("Get address ...\n");

    if (wallet->getAddresses(addresses, count, coin, derivationPath, scriptType, display, usePassphrase) != 0) {
...
int main(int argc, char **argv) {
    ...
    const char *coin_str = "Bitcoin";
    const char *derivation_path_str = "";
    const char *script_type_str = "ADDRESS";
    wallet::inputScriptType script_type;
    ...
    if (argc >= 2) {
        for (j = 1; j < argc; j++) {
            ...
            if (((argv[j][0] == '-') || (argv[j][0] == '/')) && (arglen >= 2)) {
                ...
                case 's':
                    if ((j + 1 >= argc) || (argv[j + 1][0] == '-') || (argv[j + 1][0] == '/')) {
                        printf("   Option -s requires a script type "
                               "(ADDRESS|SEGWIT|P2SHSEGWIT)\n");
                        return 1;
                    }
                    script_type_str = argv[++j];
                    break;
                case 'n':
                    if ((j + 1 >= argc) || (argv[j + 1][0] == '-') || (argv[j + 1][0] == '/')) {
                ...
                case 'c':
                    if ((j + 1 >= argc) || (argv[j + 1][0] == '-') || (argv[j + 1][0] == '/')) {
                        printf("   Option -c requires a coin name\n");
                        return 1;
                    }
                    coin_str = argv[++j];
                    break;
    ...
    if (strcmp(script_type_str, "ADDRESS") == 0)
        script_type = wallet::InputScriptType_SpendAddress;
    else if (strcmp(script_type_str, "SEGWIT") == 0)
        script_type = wallet::InputScriptType_SpendWitness;
    else if (strcmp(script_type_str, "P2SHSEGWIT") == 0)
        script_type = wallet::InputScriptType_SpendP2SHWitness;
    else
        show_help = true;

    if (show_help || (dev_type == NULL)) {
        printf("usage: %s -t dev_type [-n channel_type] [-d debug_level] [-p "
               "derivation_path] [-s script_type] [-o count] [-c coin ] [-i] "
               "[-s] [-h]\n",
               argv[0]);
        ...
        printf("   -s script_type     : script type "
               "(ADDRESS|SEGWIT|P2SHSEGWIT), default='ADDRESS'\n");
        printf("   -c coin            : coin name, default='Bitcoin'\n");
    ...
    int rc = getAddresses(wallet, std::string(coin_str), std::string(derivation_path_str), script_type, count, display,
                          use_passphrase);
